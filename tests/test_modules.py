...
@responses.activate
def test_gocomics_index(capfd, run):
...
def test_mangadex(capfd, run):
    ...
    run('--numstrips', '2', '--adult', 'MangaDex/InterspeciesReviewers')
...
@responses.activate
def test_mangadex_gone(capfd, run):
    ...
    run('MangaDex/DragonDrive', expected=1)
