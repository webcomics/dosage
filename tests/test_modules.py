...
class TestModules:
    ...
    def run(self, *options, expected: int = 0):
        """'Fake' run dosage with given options."""
        assert cmd.main(("--allow-multiple", '--basepath', str(self.tmpdir)) +
            options) == expected
    ...
    def test_turnoff(self):
        ...
        self.run('--numstrips', '2', 'turnoff')

    @responses.activate
    def test_gocomics_index(self):
        ...
        self.run('CalvinAndHobbesEnEspanol')
        self.run('CalvinAndHobbesEnEspanol:2012/07/22')

    @responses.activate
    def test_unsounded(self, capfd):
    ...
    @responses.activate
    def test_deathbulge(self, capfd):
        ...
        self.run('-v', '--numstrips', '2', 'Deathbulge')

        out = capfd.readouterr().out
        assert 'ERROR' not in out

    @responses.activate
    def test_mangadex(self, capfd):
        ...
        self.run('-v', '--numstrips', '2', '--adult', 'MangaDex/InterspeciesReviewers')

        out = capfd.readouterr().out
        assert 'ERROR' not in out

    @responses.activate
    def test_mangadex_gone(self, capfd):
        ...
        self.run('-v', 'MangaDex/DragonDrive', expected=1)
